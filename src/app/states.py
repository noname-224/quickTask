...
class BotStateMachine:
    def __init__(
        self, user_id, message: Message | None = None, call: CallbackQuery | None = None
    ):
        self.__states: dict[BotState, IState] = {
            BotState.STATE_MANAGER: StateManager(self),
            BotState.STATE_TASK_ADDITION_TITLE_INPUT_REQUEST: StateTaskAdditionTitleInputRequest(
                self
            ),
            BotState.STATE_TASK_ADDITION_TITLE_INPUT_RESPONSE: StateTaskTitleAdditionInputResponse(
                self
            ),
            BotState.STATE_TASK_ADDITION_DESCRIPTION_INPUT_REQUEST: StateTaskAdditionDescriptionInputRequest(
                self
            ),
            BotState.STATE_TASK_ADDITION_DESCRIPTION_INPUT_RESPONSE: StateTaskAdditionDescriptionInputResponse(
                self
            ),
            BotState.STATE_TASK_ADDITION: StateTaskAddition(self),
            BotState.STATE_TASK_EDITING_TITLE_INPUT_REQUEST: StateTaskEditingTitleInputRequest(
                self
            ),
            BotState.STATE_TASK_EDITING_TITLE_INPUT_RESPONSE: StateTaskEditingTitleInputResponse(
                self
            ),
            BotState.STATE_TASK_EDITING_DESCRIPTION_INPUT_REQUEST: StateTaskEditingDescriptionInputRequest(
                self
            ),
            BotState.STATE_TASK_EDITING_DESCRIPTION_INPUT_RESPONSE: StateTaskEditingDescriptionInputResponse(
                self
            ),
    ...
    def handle(self) -> None:
        if self.bsm.message is None:
            bot.answer_callback_query(
                callback_query_id=self.bsm.call.id, text=f"Сначала завершите добавление"
            )
            return
...
class StateTaskEditingTitleInputResponse(IState):
    ...
    def handle(self) -> None:
        if self.bsm.message is None:
            bot.answer_callback_query(
                callback_query_id=self.bsm.call.id, text=f"Сначала завершите изменение"
            )
            return
        ...
        self.bsm.state = state
        self.bsm.user_repo.update_state_and_context(
            user_id=self.bsm.user.id, state=state, context={"title": f"{title}"}
        )
