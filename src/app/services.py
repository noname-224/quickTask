...
from domain.enums import (
    CancelledOperationName,
    MessageUploadMethod,
    TaskAttributeText,
    TaskStatus,
)
from domain.types import MessageId, TaskId
from utils.helpers import get_message_id, get_task_id, text_for_reply_to_bad_input
...
class WindowLoaderService:
    ...
    def load_checklist(
        ...
        if upload_method.value:
            bot.send_message(
                ...
                text=text,
                reply_markup=InlineKeyboardCreator.create_checklist_window_buttons(
                    tasks
                ),
            )
        else:
    ...
    @classmethod
    def load_task(
        cls, task_id: TaskId, message: Message, upload_method: MessageUploadMethod
    ) -> None:
    ...
    def load_task_edit(cls, task_id: TaskId, message: Message) -> None:
        ...
        if task is not None:
            bot.edit_message_text(
                ...
                reply_markup=InlineKeyboardCreator.create_task_edit_window_buttons(
                    task.id
                ),
            )
        else:
...
class TaskModifierService:
    ...
    def add(cls, message: Message) -> None:
        ...
        bot.send_message(
            ...
            reply_markup=InlineKeyboardCreator.create_cancel_adding_button(
                start_msg_id
            ),
        )
        bot.register_next_step_handler(
            message, lambda msg: cls.__add_second_step(msg, start_msg_id)
        )
    ...
    def __add_second_step(cls, message: Message, start_msg_id: MessageId) -> None:
        ...
        bot.register_next_step_handler(
            message, lambda msg: cls.__add_final_step(msg, title, start_msg_id)
        )

    @classmethod
    def __add_final_step(
        cls, message: Message, title: str, start_msg_id: MessageId
    ) -> None:
        if message.content_type != "text" or message.text.startswith("/"):
            bot.reply_to(
                message=message,
        ...
        cls.task_repo.add(title=title, description=description, user_id=message.chat.id)
    ...
    def edit_title(cls, call: CallbackQuery) -> None:
        ...
        bot.send_message(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardCreator.create_cancel_editing_button(
                start_msg_id
            ),
        )
        bot.register_next_step_handler(
    ...
    @classmethod
    def __edit_title_final_step(
        cls, message: Message, task_id: TaskId, start_msg_id: MessageId
    ) -> None:
        if message.content_type != "text" or message.text.startswith("/"):
            bot.reply_to(
                message=message,
        ...
        title = message.text
        cls.task_repo.edit(task_id=task_id, title=title)
    ...
    @classmethod
    def __edit_description_final_step(
        cls, message: Message, task_id: TaskId, start_msg_id: MessageId
    ) -> None:
        if message.content_type != "text" or message.text.startswith("/"):
            bot.reply_to(
            ...
            bot.register_next_step_handler(
                message,
                lambda msg: cls.__edit_description_final_step(
                    msg, task_id, start_msg_id
                ),
            )
            return

        description = message.text
        cls.task_repo.edit(task_id=task_id, description=description)
    ...
    @classmethod
    def __edit_all_second_step(
        cls, message: Message, task: Task, start_msg_id: MessageId
    ) -> None:
        if message.content_type != "text" or message.text.startswith("/"):
            bot.reply_to(
                message=message,
            ...
            bot.register_next_step_handler(
                message, lambda msg: cls.__edit_all_second_step(msg, task, start_msg_id)
            )
            return
    ...
    @classmethod
    def __edit_all_final_step(
        cls, message: Message, task: Task, start_msg_id: MessageId, title: str
    ) -> None:
        if message.content_type != "text" or message.text.startswith("/"):
            bot.reply_to(
                message=message,
        ...
        description = message.text
        cls.task_repo.edit(task_id=task.id, title=title, description=description)
    ...
    @staticmethod
    def cancel_modify(
        call: CallbackQuery, cancelled_operation_name: CancelledOperationName
    ) -> None:
        bot.clear_step_handler_by_chat_id(call.message.chat.id)
        start_msg_id = get_message_id(call.data)
        bot.delete_messages(
            call.message.chat.id, list(range(start_msg_id, start_msg_id + 20))
        )
        bot.answer_callback_query(
            callback_query_id=call.id, text=f"{cancelled_operation_name} отменено"
        )
